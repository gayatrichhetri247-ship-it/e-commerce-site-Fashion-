...
const ProductCard = ({ product }) => {
 ...
 const handleRemoveProduct = () => {
  if (!product?.id) {
    console.warn("Cannot remove product without an id", product);
    return;
  }
    ...
    <div className="w-full overflow-hidden rounded-xl border border-gray-100 bg-white shadow-md shadow-pink-200/40 hover:shadow-lg hover:shadow-pink-200/60 transition-all duration-300 flex flex-col group">
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-gray-50">
        ...
        <button
          ...
          className="absolute top-2 right-2 bg-white/90 hover:bg-red-500 hover:text-white p-2 rounded-full shadow-md transition"
        >
          <Trash2 size={16} />
        </button>
      </div>
